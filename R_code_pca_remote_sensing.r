R code for pca remote sensing data analysis
...
We are going make use of two libraries: Raster and RStoolbox
Recall the dataset:
library(raster)
library(RStoolbox)
...
Let's see if the bands are corelated to each other or not
First of all, what does it mean corelated?
Being corelated - means you are following the pattern of another variable
Example the number of trips and the number of infected by covid are related 

Lets see if B1 and B3 are corelated to each other
Blue and Red should have a high corelation. Because this light is used to fixate photosynthesis
...
Here you have the names of the different bands
...
Positive correlation between variables. Both virables are increasing with respect to the other
Negative correlation between variables. One virable is decreasing with respect to the other
No coorelation - flat curve/line

We can apply
Principle component = PC1 - accounting for 90% 
Principle component = PC2 - accounting for 10%
...
Raster PCA is the function that performs Principle Component Analysis
Assign the image a new name: <-
...
Let us plot the map
The dollar symbol is linkingen the different outputs together with the map
plot(p224r63_2011_pca$map)

Lets make a new color ramp palette
Assign the image a new name: <-
Give it the name cl
...
Let us see how much the different bands are corelating to each other
pairs(p224r63_2011)
...
The second component accounting for ... %
The third component accounting for .... %
...
First decrease the amount of data by factor 10
Make use of the function: aggregate()
Second make use the pca
Third: plot in RGB our map
...
First we decreased the complexity of the images by facotor 10 to make it easier to plot
